...
__attribute__(( naked )) int prt(const char *a)
{
	...
	(
			...
			// r7: keeps the count of characters
			...
			"add r7, r7, #1 \n" // increment character counter by 1 and store it in r7 (don't count the terminating null)

			"compare_to_A: \n" // 'branch' compare
			"cmp r6, #64 \n" // compare the value from r6 to 64 (65 is ASCII code of first capital letter - 'A')
			"bhi compare_to_Z \n" // if it is unsigned (it is according to the assignment) and higher - jump to 'compare_to_Z' branch
			"b print \n" // otherwise jump to print branch

			"compare_to_Z: \n" // 'compare_to_Z' branch
			"cmp r6, #90 \n" // compare the value from r6 to '90' (ASCII code for last capital letter - 'Z')
			"bls lowercase\n" // if it is less or equal - jump to branch lowercase
			...
			"mov r0, r6 \n" // copy the value stored in r2 to r0 (putchar uses r0 for printing characters)
			"bl putchar \n"
			"b repeat \n" // repeat the cycle
			...
			"mov r0, r7 \n" // copy the character count from r7 and store in r0
